...
- **`skill-tree-roadmap.html`** — skill tree for the security org, branching into Penetration Testing, Network Engineering, Cybersecurity Analyst (SOC/Blue Team), Auditing, GRC, Cloud Security, and Incident Response/DFIR. Branches scroll horizontally with momentum + rubber-band bounce at the edges. **Every tool, cert, and level is individually clickable** (97 nodes total) — opens a roadmap.sh-style side panel with an explanation, a time estimate, and a link to the official source. 70 of those nodes share progress state with `tracker.html` (mark done in either place, it syncs).
- **`search.html`** — searches every tool, cert, glossary term, specialty, and page in one place (118 indexed items), with type filters. Deep-links straight into the right panel/page.
- **`cheat-sheet.html`** — one printable page summarizing all 7 specialties: top cert, cost (USD + Naira), salary range, time investment.
...
Every page has a dark/light mode toggle (sun/moon button, top-right) — it defaults to the visitor's system preference and remembers their choice via `localStorage`.
...
```bash
git remote set-url origin https://github.com/securevast/security-learning-roadmap.git
